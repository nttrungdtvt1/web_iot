...
function getBadge(log) {
  if (log.status === "granted" && log.method === "pin")
    return ["badge-pin", "Vào bằng PIN"];
  if (log.status === "granted") return ["badge-granted", "Vào được"];
  if (log.status === "denied" && log.notes?.includes("3"))
    return ["badge-alarm", "Alarm"];
...
function DetailModal({ log, onClose }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `snapshot_${log.id}.jpg`;
      a.click();
    ...
    <div className="modal-overlay" onClick={onClose}>
      ...
      >
        ...
        >
          {log.image_url ? (
            ...
            <>
              ...
              >
                Ảnh chụp từ camera lúc sự kiện xảy ra
                <br />
                <span style={{ fontSize: "0.7rem" }}>
                  (lưu trên S3 / Cloudinary)
                </span>
              </p>
            </>
          )}
        ...
        <button
          ...
          disabled={!log.image_url || downloading}
        >
...
export default function AccessLogs() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      fetchLogs({ page, limit, date: dateFilter, status: statusFilter }),
